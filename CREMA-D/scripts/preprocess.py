...
def extract_audio_features(audio_path):
    ...
    frame_rate = 28.8  # in frames per second
...
def preprocess_all_videos(source_dir, processed_output, target_dir):
    ...
    audio_features, visual_features, labels_list, actor_list = [], [], [], []
    ...
    for root, dirs, files in os.walk(source_dir):
        for filename in tqdm(files, desc="Processing videos"):
            if filename.endswith(".avi"):
                ...
                # Skip files where the last part is "XX"
                last_part = filename.split('_')[-1].split('.')[0]
                if last_part == "XX":
                    print(f"Skipping file {filename} due to last part being 'XX'.")
                    continue

                # Extract the actor and emotion label from the filename
                actor_code = filename.split('_')[0]
                ...
                # skip wrong dimension extraction
                if not isinstance(openface_feat.shape[0], int):
                    print(f"Skipping file {filename} due to mismatched dimensions.")
                    continue 
    ...
    np.save(os.path.join(target_dir, 'actor.npy'), np.array(actor_list))

    print(f"Extracted features and labels have been saved to {target_dir}")

# Usage
source_dir = 'avi'  # Folder containing the .avi files

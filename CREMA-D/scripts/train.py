...
def load_train_val_data(apply_pca=True, pca_audio_components=200, pca_visual_components=35):
    ...
    val_visual = np.load('ExtractedFeatures/val_visual.npy', allow_pickle=True)
    val_visual = val_visual.reshape(val_visual.shape[0], -1, val_visual.shape[-1])  # Flatten spatial dimensions
    val_labels = np.load('ExtractedFeatures/val_labels.npy', allow_pickle=True)

    # Load the audio data
    train_audio = np.load('ExtractedFeatures/train_audio.npy', allow_pickle=True)
    train_audio = train_audio.transpose(0, 2, 1)  # Swap feature and frames dimension
    val_audio = np.load('ExtractedFeatures/val_audio.npy', allow_pickle=True)
    ...
    if apply_pca:
        ...
        pca_audio = PCA(n_components=pca_audio_components)
        ...
        pca_visual = PCA(n_components=pca_visual_components)
    ...
    train_visual_selected = train_visual[:, :, :35]
    val_visual_selected = val_visual[:, :, :35]

    # Concatenate visual and (possibly reduced) audio features
    train_combined = np.concatenate((train_visual_selected, train_audio_reduced), axis=2)
    val_combined = np.concatenate((val_visual_selected, val_audio_reduced), axis=2)
    ...
    print(f"data shape: {train_combined.shape}")
...
def train_model(train_loader, val_loader):
    ...
    history = {
        'train_loss': [], 'train_accuracy': [], 'train_top2_accuracy': [],
        'val_loss': [], 'val_accuracy': [], 'val_top2_accuracy': [],
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader:
            ...
            top2_correct_samples += (top2_predictions == labels.view(-1, 1)).sum().item()

            total_samples += labels.size(0)

        train_accuracy = 100 * correct_samples / total_samples
        train_top2_accuracy = 100 * top2_correct_samples / total_samples

        history['train_loss'].append(running_loss / len(train_loader))
        history['train_accuracy'].append(train_accuracy)
        history['train_top2_accuracy'].append(train_top2_accuracy)

        writer.add_scalar('Loss/train', running_loss / len(train_loader), epoch)
        writer.add_scalar('Accuracy/train', train_accuracy, epoch)
        writer.add_scalar('Top-2 Accuracy/train', train_top2_accuracy, epoch)
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                ...
                # Top-2 accuracy
                top2_predictions = torch.topk(outputs, 2, dim=1).indices
                val_top2_correct += (top2_predictions == labels.view(-1, 1)).sum().item()
        ...
        val_top2_accuracy = 100 * val_top2_correct / val_total
        ...
        history['val_top2_accuracy'].append(val_top2_accuracy)
        ...
        writer.add_scalar('Top-2 Accuracy/validation', val_top2_accuracy, epoch)
        ...
        if average_val_loss < best_val_loss:
            ...
            best_val_top2_accuracy = val_top2_accuracy
            torch.save(model.state_dict(), 'best_model_parameters.pth')

        scheduler.step()

        print(f"Epoch [{epoch+1}/{num_epochs}]: Train Loss: {running_loss / len(train_loader):.4f}, "
              f"Train Accuracy: {train_accuracy:.2f}%, Train Top-2 Accuracy: {train_top2_accuracy:.2f}%, "
              f"Val Loss: {average_val_loss:.4f}, Val Accuracy: {val_accuracy:.2f}%, "
              f"Val Top-2 Accuracy: {val_top2_accuracy:.2f}%")

    writer.close()
    print(f"Best Validation Loss: {best_val_loss:.4f}, Best Validation Accuracy: {best_val_accuracy:.2f}%, "
          f"Best Validation Top-2 Accuracy: {best_val_top2_accuracy:.2f}%")
